...
function head({ title, description, url, image, type = 'website', robots = 'index, follow' }) {
  let rawTitle = title || DEFAULT_TITLE;
  
  // Bersihkan format lama jika sudah ada agar tidak ganda atau berantakan
  rawTitle = rawTitle.replace(/^(\(\*Xem Phim!\)\s*)+/, '').trim();
  rawTitle = rawTitle.replace(/(- Xem tại PhimFlix4K|Full HD Vietsub Miễn Phí Online).*$/, '').trim();

  // Susun ulang format title persis seperti yang diinginkan: (*Xem Phim!) JUDUL FILM -tahun film Full HD Vietsub Miễn Phí Online
  const finalTitle = `(*Xem Phim!) ${rawTitle} Full HD Vietsub Miễn Phí Online`;
