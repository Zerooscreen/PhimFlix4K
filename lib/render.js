...
const SITE_NAME = 'PhimFlix4K';
const DEFAULT_TITLE = '(*Xem Phim!) PhimFlix4K - Xem Phim Full HD Vietsub Miễn Phí Online';
const DEFAULT_DESC = 'Xem phim bản đẹp Full HD, Vietsub Thuyết Minh đầy đủ, cập nhật nhanh nhất tại PhimFlix4K.';
const DEFAULT_OG_IMAGE = 'https://placehold.co/1200x630/17171b/8d8a92?text=PhimFlix4K';
...
function head({ title, description, url, image, type = 'website', robots = 'index, follow' }) {
  ...
  // Bersihkan semua kotoran awalan/akhiran yang biasa dikirim dari file router
  cleanTitle = cleanTitle
    .replace(/^\(\*Xem Phim!\)\s*/i, '')
    .replace(/^\[Phim Bộ\]\s*/i, '')
    .replace(/^\[Phim\]\s*/i, '')
    .replace(/(- Xem tại PhimFlix4K|Full HD Vietsub Miễn Phí Online).*$/gi, '')
    .trim();

  // Jika title kosong (misal halaman utama), gunakan default
  let finalTitle = '';
  if (!cleanTitle || cleanTitle.toLowerCase() === 'phimflix4k') {
    finalTitle = '(*Xem Phim!) PhimFlix4K - Xem Phim Full HD Vietsub Miễn Phí Online';
  } else {
    // Format mutlak wajib: (*Xem Phim!) JUDUL FILM Full HD Vietsub Miễn Phí Online
    finalTitle = `(*Xem Phim!) ${cleanTitle} Full HD Vietsub Miễn Phí Online`;
  }
...
<header>
  <div class="header-inner">
    <a class="logo" href="/movie">PhimFlix<span>4K</span></a>
    <nav class="tabs">
...
<footer>
  <p>PhimFlix4K — Trang thông tin phim và phim bộ sử dụng dữ liệu công khai từ TMDB (không phải dịch vụ xem phim trực tuyến) · Powered by <a href="https://www.themoviedb.org/" target="_blank" rel="noopener">TMDB</a></p>
  ${histatsSnippet()}
</footer>
