...
const DEFAULT_TITLE = '(*Xem Phim!) — PhimFlix4K Full HD Vietsub Miễn Phí Online';
...
function head({ title, description, url, image, type = 'website', robots = 'index, follow' }) {
  ...
  // Bersihkan semua sisa teks kotor, awalan lama, atau duplikasi dari router/controller
  cleanTitle = cleanTitle
    .replace(/(\(\*Xem Phim!\)\s*)+/gi, '')
    .replace(/(Xem Phim!\s*[-—]\s*)+/gi, '')
    .replace(/^\[Phim Bộ\]\s*/i, '')
    .replace(/^\[Phim\]\s*/i, '')
    .replace(/([-—]\s*Xem tại PhimFlix4K|Full HD Vietsub Miễn Phí Online).*$/gi, '')
    .trim();

  let finalTitle = '';
  if (!cleanTitle || cleanTitle.toLowerCase() === 'phimflix4k') {
    finalTitle = '(*Xem Phim!) — PhimFlix4K Full HD Vietsub Miễn Phí Online';
  } else {
    // Format ketat dengan em-dash: (*Xem Phim!) — JUDUL FILM Full HD Vietsub Miễn Phí Online
