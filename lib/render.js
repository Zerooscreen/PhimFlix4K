...
function head({ title, description, url, image, type = 'website', robots = 'index, follow' }) {
  ...
  // Pastikan format selalu berawalan (*Xem Phim!) dan berakhiran Full HD Vietsub Miễn Phí Online jika itu judul film
  if (!rawTitle.startsWith('(*Xem Phim!)')) {
    rawTitle = `(*Xem Phim!) ${rawTitle} Full HD Vietsub Miễn Phí Online`;
  }

  const t = escapeHtml(rawTitle);
